...
func TestTask_RequiresTest(t *testing.T) {
	tests := []struct {
		...
		{"default (no tags)", nil, true},
		{"skip-tests tag", []string{"skip-tests"}, false},
		{"no-tests tag", []string{"no-tests"}, false},
		{"other tags", []string{"backend", "api"}, true},
	}
...
func TestTaskStore_List_WithFilter(t *testing.T) {
	...
	store.Create(&Task{Title: "A", Status: TaskReady, Priority: 10})
	store.Create(&Task{Title: "B", Status: TaskCompleted, Priority: 20})
	store.Create(&Task{Title: "C", Status: TaskReady, Priority: 5})
...
func TestTaskStore_ListByParent(t *testing.T) {
	...
	store.Create(&Task{Title: "Child A", ParentID: parent.ID, Depth: 1, Priority: 20})
	store.Create(&Task{Title: "Child B", ParentID: parent.ID, Depth: 1, Priority: 10})
...
func TestTaskStore_CancelChildren(t *testing.T) {
	...
	store.Create(&Task{Title: "Child Ready", ParentID: parent.ID, Depth: 1})
	store.Create(&Task{Title: "Child Completed", ParentID: parent.ID, Depth: 1, Status: TaskCompleted})
...
func TestTaskStore_ArchiveChildren(t *testing.T) {
	...
	store.Create(&Task{Title: "Child Pending", ParentID: parent.ID, Depth: 1, Status: TaskPending})
	store.Create(&Task{Title: "Child Ready", ParentID: parent.ID, Depth: 1, Status: TaskReady})
	store.Create(&Task{Title: "Child Running", ParentID: parent.ID, Depth: 1, Status: TaskRunning})
	store.Create(&Task{Title: "Child Completed", ParentID: parent.ID, Depth: 1, Status: TaskCompleted})
	store.Create(&Task{Title: "Child Failed", ParentID: parent.ID, Depth: 1, Status: TaskFailed})
	store.Create(&Task{Title: "Child Cancelled", ParentID: parent.ID, Depth: 1, Status: TaskCancelled})
...
func TestTaskStore_ArchiveChildren_AlreadyArchived(t *testing.T) {
	...
	store.Create(&Task{Title: "Child Archived", ParentID: parent.ID, Depth: 1, Status: TaskArchived})
	store.Create(&Task{Title: "Child Ready", ParentID: parent.ID, Depth: 1, Status: TaskReady})
...
func TestTaskStore_DecomposedStatus(t *testing.T) {
	...
	task := &Task{Title: "Decomposed Task", Status: TaskDecomposed}
	if err := store.Create(task); err != nil {
		t.Fatalf("Create: %v", err)
	}
...
func TestTaskStore_ListPending(t *testing.T) {
	...
	store.Create(&Task{Title: "Pending 1", Source: TaskSourceOperator})
	store.Create(&Task{Title: "Ready 1", Status: TaskReady})
...
func TestTaskStore_List_ExcludeSubtasks(t *testing.T) {
	...
	parent := &Task{Title: "Parent"}
	store.Create(parent)
	store.Create(&Task{Title: "Child", ParentID: parent.ID, Depth: 1})
	store.Create(&Task{Title: "Top-level"})
...
func TestTaskStore_ValidateSubtaskDAG_ValidLinearChain(t *testing.T) {
	...
	taskA := &Task{Title: "A", ParentID: parent.ID, Depth: 1}
	store.Create(taskA)

	taskB := &Task{Title: "B", ParentID: parent.ID, Depth: 1, DependsOn: []string{taskA.ID}}
	store.Create(taskB)
...
func TestTaskStore_ValidateSubtaskDAG_ValidDiamond(t *testing.T) {
	...
	taskC := &Task{Title: "C", ParentID: parent.ID, Depth: 1, DependsOn: []string{taskA.ID}}
	store.Create(taskC)
...
func TestTaskStore_ValidateSubtaskDAG_RejectThreeNodeCycle(t *testing.T) {
	...
	taskC := &Task{Title: "C", ParentID: parent.ID, Depth: 1, DependsOn: []string{taskB.ID}}
	store.Create(taskC)
...
func TestTaskStore_GetTopologicalOrder_LinearChain(t *testing.T) {
	...
	// Create A -> B -> C
	taskA := &Task{Title: "A", ParentID: parent.ID, Depth: 1, Priority: 10}
	store.Create(taskA)

	taskB := &Task{Title: "B", ParentID: parent.ID, Depth: 1, Priority: 20, DependsOn: []string{taskA.ID}}
	store.Create(taskB)

	taskC := &Task{Title: "C", ParentID: parent.ID, Depth: 1, Priority: 30, DependsOn: []string{taskB.ID}}
	store.Create(taskC)
...
func TestTaskStore_GetTopologicalOrder_Diamond(t *testing.T) {
	...
	taskD := &Task{Title: "D", ParentID: parent.ID, Depth: 1, DependsOn: []string{taskB.ID, taskC.ID}}
	store.Create(taskD)
...
func TestTaskStore_GetTopologicalOrder_DisconnectedSubgraphs(t *testing.T) {
	...
	taskC := &Task{Title: "C", ParentID: parent.ID, Depth: 1}
	store.Create(taskC)

	taskD := &Task{Title: "D", ParentID: parent.ID, Depth: 1, DependsOn: []string{taskC.ID}}
	store.Create(taskD)
...
func TestTaskStore_AddSubtaskDependency_Success(t *testing.T) {
	...
	taskB := &Task{Title: "B", ParentID: parent.ID, Depth: 1}
	store.Create(taskB)
...
func TestTaskStore_AddSubtaskDependency_DifferentParents(t *testing.T) {
	...
	parent1 := &Task{Title: "Parent1"}
	store.Create(parent1)

	parent2 := &Task{Title: "Parent2"}
	store.Create(parent2)

	taskA := &Task{Title: "A", ParentID: parent1.ID, Depth: 1}
	store.Create(taskA)

	taskB := &Task{Title: "B", ParentID: parent2.ID, Depth: 1}
	store.Create(taskB)

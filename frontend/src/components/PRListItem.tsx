      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5">
            <h3 className="text-sm font-medium text-white/90 truncate">{pr.title}</h3>
            <PRStatusBadge status={pr.pr_status} />
          </div>
          {pr.description && (
            <button type="button" className={`mb-2 w-full text-left hover:bg-white/[0.02] rounded-lg p-1 -ml-1 transition-colors ${isDescriptionExpanded ? '' : 'max-h-12 overflow-hidden'}`} onClick={onToggle} aria-expanded={isDescriptionExpanded} aria-label={isDescriptionExpanded ? 'Collapse description' : 'Expand description'}>
              <MarkdownRenderer content={pr.description} />
              {!isDescriptionExpanded && <div className="text-[10px] text-white/20 mt-0.5">Click to expand</div>}
            </button>
          )}
          <div className="flex items-center gap-2.5 text-[11px] text-white/25 flex-wrap">
            {pr.diff_lines !== undefined && <span><span className="text-emerald-400">+</span><span className="text-rose-400">-</span> {pr.diff_lines}L / {pr.files_changed}F</span>}
            {pr.branch_name && <span className="font-mono text-white/20 bg-white/[0.04] px-1.5 py-0.5 rounded text-[10px]">{pr.branch_name}</span>}
            {pr.model && <span className="text-white/15">{pr.model}</span>}
            {pr.created_at && <span>{timeAgo(pr.created_at)}</span>}
          </div>
          <div className="flex items-center gap-3 mt-2 flex-wrap">
            {pr.pr_url && (
              <a href={pr.pr_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-accent-400 hover:text-accent-300 text-[11px] transition-colors">

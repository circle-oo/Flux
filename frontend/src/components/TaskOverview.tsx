    ...
    <>
      <div className="card p-5">
        <div className="flex items-center gap-2 mb-3">
          <h2 className="text-sm font-semibold text-content">Description</h2>
          {task.triage_description && <span className="bg-surface-hover text-content-faint px-1.5 py-0.5 rounded text-[10px] font-medium">User</span>}
        </div>
      ...
      {task.triage_description && (
        <div className="card p-5 ring-1 ring-cyan-500/15">
          <div className="flex items-center gap-2 mb-3">
            <h2 className="text-sm font-semibold text-cyan-600">Triage Description</h2>
            <span className="badge-info">AI</span>
          </div>
          <div className="text-sm text-content-secondary leading-relaxed"><MarkdownRenderer content={task.triage_description} /></div>
        </div>
      )}
      <div className="card p-5">
        ...
        {task.parent_id && <InfoRow label="Parent Task"><button onClick={() => navigate(`/tasks/${task.parent_id}`)} className="font-mono text-xs text-primary-400 hover:text-primary-300 transition-colors">{task.parent_id.slice(0, 8)}</button>{task.depth !== undefined && <span className="ml-2 text-content-faint">(depth {task.depth})</span>}</InfoRow>}
        {task.depends_on.length > 0 && <InfoRow label="Depends On"><div className="flex flex-wrap gap-1">{task.depends_on.map((dep) => (<button key={dep} onClick={() => navigate(`/tasks/${dep}`)} className="font-mono text-xs text-primary-400 hover:text-primary-300 transition-colors">{dep.slice(0, 8)}</button>))}</div></InfoRow>}
